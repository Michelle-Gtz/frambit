...
const ProjectForm = () => {
  ...
  const onInvokeAi = async () => {
    try {
      ...
      console.log(res);
      toast.success("Invoked AI successfully");
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {PROJECT_TEMPLATES.map((template, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
          </button>
        ))}
      </div>
      ...
      <Form {...form}>
        <form
          ...
          className={cn(
            "relative border rounded-xl bg-sidebar dark:bg-sidebar px-4 pt-2 pb-3 transition-all",
          ...
          <FormField
            ...
            render={({ field }) => (
              <TextAreaAutosize
                ...
                maxRows={8}
                className={cn(
                  "w-full resize-none bg-transparent outline-none border-none pt-3 text-sm text-foreground placeholder:text-muted-foreground",
                )}
                onKeyDown={(e) => {
